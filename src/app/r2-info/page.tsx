...
async function getR2Info() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch R2 info: ${response.status} ${response.statusText}`);
    }
        ...
        <div className="space-y-6">
          <div className="p-4 border rounded-lg">
            ...
            <div className="space-y-2">
              <div><strong>Bucket Name:</strong> {r2Info.bucketInfo.name}</div>
              <div><strong>Endpoint:</strong> {r2Info.bucketInfo.endpoint}</div>
              <div><strong>Public URL:</strong> {r2Info.bucketInfo.publicUrl}</div>
            </div>
          </div>
          
          <div className="p-4 border rounded-lg">
            <h2 className="text-xl font-semibold mb-4">Objects in Bucket (Limited to 10)</h2>
            {r2Info.objects.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {r2Info.objects.map((obj: any) => (
                      <tr key={obj.key}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{obj.key}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{obj.size} bytes</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {new Date(obj.lastModified).toLocaleString()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-500">
                          <a href={obj.publicUrl} target="_blank" rel="noopener noreferrer">View</a>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className="text-gray-500">No objects found in the bucket.</div>
            )}
          </div>
          
          <div className="p-4 border rounded-lg">
            <h2 className="text-xl font-semibold mb-4">Public Access Test Results</h2>
            {r2Info.accessResults.length > 0 ? (
              <div className="space-y-4">
                {r2Info.accessResults.map((result: any) => (
                  <div key={result.key} className="p-3 border rounded">
                    <div><strong>Object Key:</strong> {result.key}</div>
                    <div><strong>Public URL:</strong> <a href={result.publicUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500">{result.publicUrl}</a></div>
                    <div>
              ...
              <div className="text-gray-500">No access test results available.</div>
            )}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <ul className="list-disc pl-5 mt-1">
                  ...
                  <li>Allowed Origins: <code>http://localhost:3000</code>, your production domain</li>
              ...
              <div>
                ...
                <p>When uploading files, make sure to set the ACL to "public-read":</p>
...
{`const params = {
  ...
  ACL: "public-read"  // This is important!
        ...
        <div className="p-4 border rounded-lg bg-red-50 text-red-600">
          <h2 className="text-xl font-semibold mb-2">Error Fetching R2 Information</h2>
