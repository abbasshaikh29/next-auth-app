/**
 * Test script for AWS S3 integration
 *
 * This script:
 * 1. Creates a simple test image
 * 2. Uploads it to S3
 * 3. Retrieves and displays the URL
 * 4. Tests if the image is accessible
 */

import dotenv from "dotenv";
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import axios from "axios";
import {
  S3Client,
  PutObjectCommand,
  GetObjectCommand,
} from "@aws-sdk/client-s3";

// Initialize dotenv
dotenv.config({ path: ".env.local" });

// Get __dirname equivalent in ES modules
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Initialize S3 client
const s3Client = new S3Client({
  region: process.env.AWS_REGION || "ap-southeast-1",
  credentials: {
    accessKeyId: process.env.AWS_ACCESS_KEY_ID || "",
    secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || "",
  },
});

const bucketName = process.env.AWS_S3_BUCKET || "";

// Function to generate a test image (a simple 100x100 colored square)
function generateTestImage() {
  const testImagePath = path.join(__dirname, "test-image.png");

  // Check if we already have a test image
  if (fs.existsSync(testImagePath)) {
    console.log("Using existing test image");
    return testImagePath;
  }

  // For simplicity, we'll just copy a placeholder image
  // In a real scenario, you might want to generate an image programmatically
  console.log("Creating a test image...");

  // Create a simple text file as a placeholder
  fs.writeFileSync(testImagePath, "This is a test file for S3 upload");

  return testImagePath;
}

// Function to upload a file to S3
async function uploadToS3(filePath, key, contentType) {
  console.log(`Uploading ${filePath} to S3...`);

  const fileContent = fs.readFileSync(filePath);

  const params = {
    Bucket: bucketName,
    Key: key,
    Body: fileContent,
    ContentType: contentType,
  };

  try {
    await s3Client.send(new PutObjectCommand(params));
    console.log("Upload successful");

    // Generate the URL
    let url;
    if (process.env.CLOUDFRONT_DOMAIN) {
      url = `https://${process.env.CLOUDFRONT_DOMAIN}/${key}`;
    } else if (process.env.NEXT_PUBLIC_S3_URL) {
      url = `${process.env.NEXT_PUBLIC_S3_URL}/${key}`;
    } else {
      url = `https://${bucketName}.s3.${
        process.env.AWS_REGION || "ap-southeast-1"
      }.amazonaws.com/${key}`;
    }

    return url;
  } catch (error) {
    console.error("Error uploading to S3:", error);
    throw error;
  }
}

// Function to test if an image URL is accessible
async function testImageAccess(url) {
  console.log(`Testing access to ${url}...`);

  try {
    const response = await axios.get(url);
    console.log(`Image is accessible! Status: ${response.status}`);
    return true;
  } catch (error) {
    console.error(`Error accessing image: ${error.message}`);
    if (error.response) {
      console.error(`Status: ${error.response.status}`);
      console.error(`Headers:`, error.response.headers);
    }
    return false;
  }
}

// Main test function
async function testS3Integration() {
  console.log("Testing AWS S3 integration...");
  console.log("---------------------------");
  console.log(`AWS Region: ${process.env.AWS_REGION}`);
  console.log(`S3 Bucket: ${bucketName}`);
  console.log(
    `Public S3 URL: ${process.env.NEXT_PUBLIC_S3_URL || "Not configured"}`
  );
  console.log("---------------------------");

  try {
    // Generate a test image
    const testImagePath = generateTestImage();

    // Generate a unique key for the test image
    const timestamp = Date.now();
    const key = `test/test-image-${timestamp}.txt`;

    // Upload the image to S3
    const imageUrl = await uploadToS3(testImagePath, key, "text/plain");
    console.log(`Image uploaded successfully. URL: ${imageUrl}`);

    // Test if the image is accessible
    const isAccessible = await testImageAccess(imageUrl);

    if (isAccessible) {
      console.log("✅ S3 integration test PASSED!");
      console.log(`The image is available at: ${imageUrl}`);
    } else {
      console.log("❌ S3 integration test FAILED!");
      console.log("The image was uploaded but is not publicly accessible.");
      console.log(
        "This might be due to bucket permissions or CORS configuration."
      );
    }

    // Clean up
    fs.unlinkSync(testImagePath);
    console.log("Test image removed from local filesystem");
  } catch (error) {
    console.error("Test failed:", error);
  }
}

// Run the test
testS3Integration().catch(console.error);
